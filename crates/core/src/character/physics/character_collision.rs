...
//!
//! 1. **[`spatial_cache::update_character_spatial_cache`]** — rebuilds the
...
//! # Complexity
//!
//! - Cache rebuild: O(n) — each character touches at most 4 chunks.
//! - Pair scan: O(k²) per chunk, where *k* is the local population.  Global
//!   worst case is still O(n²) when all characters pile into one chunk, but
//!   the average case is far better for spread-out populations.
//!
//! # Vertical axis
//!
...
use super::{
    Aabb, CharacterCollider, PhysicsBody, PhysicsSet, TentativePosition, Velocity,
    spatial_cache::{CharacterSpatialCache, update_character_spatial_cache},
};
...
///
/// Candidate pairs come from [`CharacterSpatialCache`], which was rebuilt by
/// [`update_character_spatial_cache`] earlier in the same
/// [`PhysicsSet::CharacterCollision`] tick.
///
...
fn resolve_character_collisions(
    ...
    // Snapshot each entity's position and dynamism flag from the immutable
    // query view, then accumulate corrections, then apply them via mutable
    // access.  The two-pass design avoids borrow-checker conflicts that would
    // arise from calling query.get() and query.get_mut() for the same entity
    // in the same scope.
    //
    // The cache provides the (Entity, Entity) candidate pairs.  We read the
    // full component state via query.get(entity), accumulate corrections into
    // a HashMap, then apply them via query.get_mut(entity).
    ...
    for (entity_a, entity_b) in cache.candidate_pairs() {
        // Either entity may have been despawned since the cache was built.
        ...
        // Strip Y — horizontal correction only.
        let horizontal = Vec3::new(pen.x, 0.0, pen.z);
        if horizontal.length_squared() < f32::EPSILON {
            continue;
        }

        // A small bias ensures entities fully separate despite floating-point
        // rounding, which is especially important when three or more characters
        // interact simultaneously in one tick.
    ...
    for (entity, correction) in corrections {
        ...
        // Zero velocity on corrected axes to prevent immediate re-penetration.
...
pub(super) struct CharacterCollisionPlugin;
...
mod tests {
    ...
    /// Spawn a fully dynamic character collider at `pos`.
    ...
    /// Two slightly-overlapping characters must be pushed apart.
    #[test]
    ...
    /// Characters that are already far apart must not be moved by the collision
    /// system.
    #[test]
    ...
    /// Both dynamic bodies must each move the same amount in opposite directions
    /// and the midpoint must remain near the original centre.
    #[test]
    ...
    /// Character collision must not alter the Y position of either entity.
    #[test]
    ...
    /// After a collision the velocity components along the corrected axes must
    /// be zeroed to prevent immediate re-penetration.
    #[test]
    ...
    /// Three overlapping characters in a line must all be non-overlapping after
    /// enough ticks to let the iterative resolution converge.
    #[test]
    ...
    /// Two characters whose AABBs straddle the same chunk boundary must still
    /// collide — the spatial cache must list them as a candidate pair from both
    /// chunks.
    #[test]
    fn characters_straddling_chunk_boundary_are_resolved() {
        let mut app = make_app(1.0 / 60.0);

        // Both characters near the X boundary between chunk (0,0) and (1,0).
    ...
    /// Characters in completely different chunks must never be paired, so they
    /// must not be moved by the collision system.
    #[test]
    ...
    /// A character that moves from one chunk into another must be tracked by
    /// the cache and collide with existing characters in the destination chunk.
    #[test]
    fn character_moving_between_chunks_still_collides() {
        let mut app = make_app(1.0 / 20.0);

        // e1 starts in chunk (0,0), e2 is in chunk (1,0).
        let e1 = spawn_character(&mut app, Vec3::new(4.0, 0.0, 4.0));
        let e2 = spawn_character(&mut app, Vec3::new(20.0, 0.0, 4.0));

        // First tick: they are in different chunks — cache has no pair.
        tick(&mut app);
        {
        ...
        // Teleport e1 next to e2 (into chunk (1,0)) so they overlap.
    ...
    /// A static obstacle (Velocity removed) must not move; the dynamic
    /// character must absorb the full correction.
    #[test]
    fn static_obstacle_does_not_move() {
        ...
        let obstacle = app
            ...
            .id();
        // Remove the Velocity that #[require] inserts so the obstacle is static.
